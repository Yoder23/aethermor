...
Apache~2.0 license with an interactive browser-based explorer, 212
automated tests, and six ready-to-run research examples.
\end{abstract}
...
\begin{equation}
  ...
  \label{eq:rstack}
\end{equation}
Twelve pre-built materials (thermal paste, liquid metal, copper IHS,
...
browser-based interactive explorer (\texttt{app.py}) with five
parameterised tabs updated in real time; (2)~a Python scripting API;
(3)~six ready-to-run example scripts demonstrating research workflows.
...
The entire test suite (212~tests) and validation suite (133~checks)
...
All results are from physics-based models calibrated to published data
(CODATA, CRC, ITRS/IRDS), not measured hardware. Validation against
infrared thermal imaging of test chips is a planned next step.
...
\begin{center}
\url{https://github.com/YOUR_ORG/aethermor}
\end{center}
...
Aethermor provides the first open-source toolkit for inverse thermal
...
design algorithms, it enables hardware architects to explore material
selections, cooling trade-offs, paradigm crossovers, and
heterogeneous SoC optimisations in seconds rather than weeks. The
